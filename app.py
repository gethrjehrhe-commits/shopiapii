...
# ==================== SESSION TOKEN EXTRACTION (FIXED) ====================

def extract_session_token(response_obj, text, unescaped, checkout_url):
    """
    Multi-layer session token extraction.
    Shopify serves the token via response header, embedded JSON bootstrap,
    meta tags, and URL hash — try all four in priority order.
    Returns token string or None.
    """
    # Layer 1: response headers (fastest, most reliable)
    for hdr in [
    ...
    # Layer 2: JSON bootstrap blocks — Shopify embeds several shapes
    token_patterns = [
        # Standard checkout-web bootstrap
        r'"serializedSessionToken"\s*:\s*"([^"]{20,})"',
        r'"sessionToken"\s*:\s*"([^"]{20,})"',
        r'"checkoutSessionToken"\s*:\s*"([^"]{20,})"',
        # Next.js / RSC payload variants
        r'session[-_]?[Tt]oken["\']?\s*:\s*["\']([^"\']{20,})["\']',
        r'"token"\s*:\s*"([a-zA-Z0-9_\-\.]{30,})"',
        # Serialized inside HTML attribute value (escaped quotes)
        r'serialized-sessionToken["\s]+content=["\']([^"\']{20,})["\']',
        r'data-session-token=["\']([^"\']{20,})["\']',
        r'data-checkout-session-token=["\']([^"\']{20,})["\']',
        # Shopify checkout one-page (2024 redesign)
        r'"checkoutToken"\s*:\s*"([^"]{20,})"',
        r'__CHECKOUT_DATA__[^{]*\{[^}]*"token"\s*:\s*"([^"]{20,})"',
        r'window\.__checkout\s*=\s*\{[^}]*"sessionToken"\s*:\s*"([^"]{20,})"',
        # Embedded script tag JSON
        r'<script[^>]*type=["\']application/json["\'][^>]*>\s*\{[^<]*"sessionToken"\s*:\s*"([^"]{20,})"',
    ]
    ...
    for src in (text, unescaped):
        for pat in token_patterns:
            ...
            if m:
                tok = m.group(1).strip()
                # Reject obvious non-tokens: pure hex checksums, build IDs
                if len(tok) >= 20 and not re.fullmatch(r'[0-9a-f]{40}', tok):
                    return tok

    # Layer 3: meta tag (older Shopify checkout)
    m = re.search(r'<meta\s+name=["\']shopify-checkout-session-token["\'][^>]*content=["\']([^"\']+)["\']', unescaped, re.IGNORECASE)
    if m:
        return m.group(1).strip()

    # Layer 4: URL-embedded token — cn/XXXXXXXX paths use the path segment as token
    m = re.search(r'/checkouts/(?:cn/)?([a-zA-Z0-9_\-]{20,})', checkout_url)
    if m:
        candidate = m.group(1)
        # Must not look like a pure numeric variant ID
...
def extract_payment_identifier(seller_proposal):
    """
    Pull the first usable direct-payment method identifier from seller_proposal.
    Handles FilledPaymentTerms, nested availablePaymentLines, and all known
    PaymentMethod __typename variants Shopify returns.
    Returns (identifier, gateway_name) or ("shopify_payments", "Shopify Payments").
    """
    ...
    typename = payment_data.get('__typename', '')
    if typename not in ('FilledPaymentTerms', ''):
        return "shopify_payments", "Shopify Payments"

    lines = payment_data.get('availablePaymentLines') or []

    # Priority order: direct card providers beat wallets/offsite
    ...
    for line in lines:
        ...
        # Skip pure wallet configs — they can't receive a vaulted card token
        if pm_typename in ('ShopPayWalletConfig', 'ApplePayWalletConfig',
                           'GooglePayWalletConfig', 'FacebookPayWalletConfig',
                           'ShopifyInstallmentsWalletConfig', 'PaypalWalletConfig',
        ...
        pm_name = (pm.get('extensibilityDisplayName') or
                   pm.get('displayName') or
                   pm.get('name') or pm_id).strip()
    ...
    # Prefer known direct-card identifiers
...
def safe_parse_json(text, label=""):
    """Parse JSON with clear error logging; returns (dict_or_None, error_str_or_None)."""
    if not text:
    ...
    try:
        ...
        return None, f"Invalid JSON ({label}): {e} — body: {snippet}"
    ...
    try:
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            if not attempt_token:
                # Try extracting from HTML (some redirects embed it)
            ...
            # --- session token extraction (hardened) ---
            sst = extract_session_token(response, text, unescaped, checkout_url)
            if not sst:
                logger.error(f"Session token extraction failed. checkout_url={checkout_url} "
                             f"headers_present={list(response.headers.keys())[:10]}")
            ...
            if not isinstance(running_total_data, dict):
                ...
                if isinstance(total_data, dict):
                    running_total = safe_get(total_data, 'value', 'amount', default="0.00")
                else:
                    running_total = "0.01"
            else:
            ...
            # --- payment identifier extraction (hardened) ---
            ...
            # If delivery proposal returns updated payment identifier, use it
            if resp_text2:
                delivery_json, _ = safe_parse_json(resp_text2, "delivery")
                if delivery_json:
                    d_seller = safe_get(delivery_json, 'data', 'session', 'negotiate', 'result', 'sellerProposal')
                    if isinstance(d_seller, dict):
                        updated_id, updated_gw = extract_payment_identifier(d_seller)
                        if updated_id != "shopify_payments":
                            payment_identifier, gateway = updated_id, updated_gw
            ...
            try:
                ...
                if vault_err or not isinstance(token_data, dict):
                    return False, f"Vault parse error: {vault_err or 'non-dict response'}", gateway, total_price, currency
            ...
            for i in range(5):
                ...
                poll_resp_json, _ = safe_parse_json(final_text, f"poll_{i}")
                if poll_resp_json:
                    receipt = safe_get(poll_resp_json, 'data', 'receipt', default={})
            ...
            if final_text and 'CAPTCHA_REQUIRED' in final_text:
                return True, "CARD_DECLINED", gateway, total_price, currency
            if final_text and 'WaitingReceipt' in final_text:
                return False, "Change Proxy or Site", gateway, total_price, currency

            final_json, _ = safe_parse_json(final_text, "final")
            if final_json:
                result_code = safe_get(final_json, 'data', 'receipt', 'processingError', 'code')
                if "shopify_payments" in str(final_json):
                    return True, "ORDER_PLACED", gateway, total_price, currency
                if result_code:
